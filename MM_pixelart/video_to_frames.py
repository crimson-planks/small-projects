...
pixelart_dimensions = (26*2-1,15*2) #the pixelarts are aligned weirdly
    ...
    while True:
        ...
        if resized_video_dimensions==None:
            ...
            aligned_pixelart_dimensions = (int(np.ceil(resized_video_dimensions[0]/8)),
                                           int(np.ceil(resized_video_dimensions[0]/8)))
            aligned_video_dimensions = (int(np.ceil(resized_video_dimensions[0]/8))*8,
                                        int(np.ceil(resized_video_dimensions[0]/8))*8)
        img = img.resize(resized_video_dimensions)
        img = img.convert("1")
        finimg: Image.Image
        if not perfect:
            finimg = Image.new('1',aligned_video_dimensions).paste(img)
        else:
            finimg = img.copy()
